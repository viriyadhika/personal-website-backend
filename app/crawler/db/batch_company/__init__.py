...
class BatchContentResult:
  def __init__(self, company: str, job_id: str, job_name: str, employee: str) -> None:
  ...
  def get_dict(self):
    return { 'company': self.company, 'job_id': self.job_id, 'job_name': self.job_name, 'employee': self.employee }


def get_batch_content(id, timestamp) -> List[BatchContentResult]:
  query = (
    f"SELECT {COMPANY_TABLE}.{CompanyColumn.name}, {JOB_TABLE}.{JobColumn.job_id}, {JOB_TABLE}.{JobColumn.name}, {COMPANY_TABLE}.{CompanyColumn.employee} " 
  ...
  with get_cursor() as wrapper:
    try:
      wrapper.cursor.execute(query, { BatchRelationshipColumn.batch_id: id, BatchRelationshipColumn.timestamp: timestamp })
      result = []
      for (company, job_id, job_name, employee_number) in wrapper.cursor:
        result.append(BatchContentResult(company=company, job_id=job_id, job_name=job_name, employee=employee_number))
      return result
